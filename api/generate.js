...
const { SCENARIO_A_PROGRESS, SCENARIO_B_WAITING, SCENARIO_C_CONFLICT } = require('./scenario-data.js');
...
function buildDemoResult(query) {
  const demoMap = {
    ...
    // === 시나리오 3종 (테스트용, 실제 Google 데이터 연동 전 데모 흐름 검증) ===
    // A — 진행 중: 9/10 기획 회의(캘린더 참고 캘린더) → 같은 날 전후 Drive·Docs·Gmail·Sheets 통합
    "MABC 2027 진행 중": SCENARIO_A_PROGRESS,
    // B — 대기 중: 캘린더 9/15 PRD 리뷰 + 9/16 마감(마감 트리거) → Gmail 알림 + Sheets 미완료
    "MABC 2027 대기 중": SCENARIO_B_WAITING,
    // C — 확인 필요·충돌: 캘린더 마감 시각(17:00~17:30, 시각 포함) vs Sheets 기한(날짜만) → 시차 / 9/15 리뷰와 연동 테스트 기한 겹침
    "MABC 2027 충돌": SCENARIO_C_CONFLICT
  };
